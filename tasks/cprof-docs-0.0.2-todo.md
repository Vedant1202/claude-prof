...
- [x] T4 Guide **Track drift** — `cprof diff <profile>` vs live (drift; vs `install --dry-run`); Mermaid profile→live→diff flowchart; sidebar + cross-link
- [x] T5 Guide **Output locations & helper files** — `init --out`/`install --into`/`--no-gitignore`/`--no-report`; Mermaid redirect flowchart + a where-files-land table; sidebar + cross-link
- [x] ⟂ Checkpoint B — all 4 guides build, in sidebar, cross-linked; Mermaid renders
